import product1Img from '../../assets/img/product-1.jpg';
import product2Img from '../../assets/img/product-2.jpg';
import product3Img from '../../assets/img/product-3.jpg';
import product4Img from '../../assets/img/product-4.jpg';
import product5Img from '../../assets/img/product-5.jpg';
import product6Img from '../../assets/img/product-6.jpg';
import product7Img from '../../assets/img/product-7.jpg';
import product8Img from '../../assets/img/product-8.jpg';
import product9Img from '../../assets/img/product-9.jpg';
import product10Img from '../../assets/img/product-10.jpg';
import product11Img from '../../assets/img/product-11.jpg';
import product12Img from '../../assets/img/product-12.jpg';

export const catalog = [
  {
    id: 1,
    brand: 'Zara',
    name: 'Camisa Larga com Bolsos',
    price: 70,
    image: product1Img,
    feminine: false,
  },
  {
    id: 2,
    brand: 'Zara',
    name: 'Casaco Reto com Lã',
    price: 85,
    image: product2Img,
    feminine: true,
  },
  {
    id: 3,
    brand: 'Zara',
    name: 'Jaqueta com Efeito Camurça',
    price: 60,
    image: product3Img,
    feminine: false,
  },
  {
    id: 4,
    brand: 'Zara',
    name: 'Sobretudo em Mescla de Lã',
    price: 160,
    image: product4Img,
    feminine: false,
  },
  {
    id: 5,
    brand: 'Zara',
    name: 'Camisa Larga Acolchoada de Veludo Cotelê',
    price: 110,
    image: product5Img,
    feminine: false,
  },
  {
    id: 6,
    brand: 'Zara',
    name: 'Casaco de Lã com Botões',
    price: 170,
    image: product6Img,
    feminine: true,
  },
  {
    id: 7,
    brand: 'Zara',
    name: 'Casaco com Botões',
    price: 75,
    image: product7Img,
    feminine: true,
  },
  {
    id: 8,
    brand: 'Zara',
    name: 'Colete Comprido com Cinto',
    price: 88,
    image: product8Img,
    feminine: true,
  },
  {
    id: 9,
    brand: 'Zara',
    name: 'Casado Curto de Pelo Alamares',
    price: 120,
    image: product9Img,
    feminine: true,
  },
  {
    id: 10,
    brand: 'Zara',
    name: 'Jaqueta Corta-Vendo Xadrex',
    price: 99,
    image: product10Img,
    feminine: false,
  },
  {
    id: 11,
    brand: 'Zara',
    name: 'Jaqueta com Gola Combinada',
    price: 85,
    image: product11Img,
    feminine: false,
  },
  {
    id: 12,
    brand: 'Zara',
    name: 'Casaco Curto de Pano',
    price: 135,
    image: product12Img,
    feminine: true,
  },
];

export const catalogIndexedById = catalog.reduce((acc, product) => {
  acc[product.id] = product;
  return acc;
}, {});
